import './Chat.css';

import React, { useState, useEffect, useRef } from "react";

import firebase from 'firebase/compat/app';
import { useCollectionData } from 'react-firebase-hooks/firestore';
import { auth, firestore } from './Firebase';
import { format } from 'timeago.js';

export function Chat() {
    const bottomMessageRef = useRef(null);
    const inputMessageRef = useRef(null);
    const messagesCollection = firestore.collection('messages');
    const query = messagesCollection.orderBy('createdAt').limitToLast(50);
    const [messages, loading, error] = useCollectionData(query, { idField: 'id', snapshotListenOptions: {} });
    const [message, setMessage] = useState('');

    let lastSize = 0;
    query.onSnapshot({
        // next: (e) => {
        //     if (e.size !== lastSize) scrollToBottom();
        // }
    })

    useEffect(() => {
        bottomMessageRef.current?.scrollIntoView({ behavior: 'smooth' })
        inputMessageRef.current?.focus();
    }, []);

    const onSubmit = async (e) => {
        e.preventDefault();
        if (!message) return;
        await messagesCollection.add({
            text: message,
            createdAt: firebase.firestore.FieldValue.serverTimestamp(),
            uid: auth.currentUser.uid,
            photoURL: auth.currentUser.photoURL,
            displayName: auth.currentUser.displayName
        })
        console.log({ auth: auth.currentUser })
        setMessage('');
        // scrollToBottom();
        inputMessageRef.current?.focus();
    }

    const inputFocus = (e) => {
        // scrollToBottom();
    }

    return (
        <chat>
            <div id="messages">
                {messages?.map((msg, i) => {
                    let timeAgoInfo, nameInfo;
                    const lastMessage = messages[i - 1];
                    const timeDiff = msg?.createdAt?.seconds - lastMessage?.createdAt?.seconds;
                    if (i === 0 || timeDiff > 60) {
                        timeAgoInfo = <p class="message-info message-date">{format(msg?.createdAt?.seconds * 1000)}</p>;
                    }
                    const differentName = msg?.displayName !== lastMessage?.displayName;
                    if (differentName) {
                        console.log('name!');
                        nameInfo = <p class="message-info">{msg?.displayName}</p>;
                    }
                    return (
                        <React.Fragment key={msg.id}>
                            {timeAgoInfo}
                            {nameInfo}
                            <ChatMessage key={msg.id} message={msg}></ChatMessage>
                        </React.Fragment>
                    )
                })}
                <div ref={bottomMessageRef}></div>
            </div>
            <form onSubmit={onSubmit}>
                <input ref={inputMessageRef} name="message" placeholder="Aa" autocomplete="off" onFocus={inputFocus} value={message} onChange={(e) => setMessage(e.target.value)}></input>
                <button type="submit">Send</button>
            </form>
        </chat>
    )
}

function ChatMessage(props) {
    const unknownPhotoURL = 'data:image/png;base64,';
    const messageWasSent = auth.currentUser.uid === props.message.uid;
    return (
        <message className={messageWasSent ? 'sent' : 'received'}>
            <img class="avatar" alt="avatar" src={messageWasSent ? auth.currentUser.photoURL : props.message.photoURL ? props.message.photoURL : unknownPhotoURL} />
            <div class="content">{props.message.text}</div>
        </message>
    )
}